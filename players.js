...
window.addEventListener('DOMContentLoaded', async () => {
   ...
   if (!tableBody || !searchInput) {
      console.error('Neki elementi nisu pronađeni u DOM-u!');
      return;
   }

   async function fetchPlayers() {
      try {
         console.log('Pokušavam učitati JSON...');
         ...
         console.log('Podaci iz JSON:', data);
         return data || [];
      } catch (error) {
         console.error('Greška pri dohvaćanju podataka:', error);
         tableBody.innerHTML = `<tr><td colspan="7" style="text-align:center; color: red;">Greška pri učitavanju podataka: ${error.message}</td></tr>`;
         return [];
      }
   }
   ...
   function renderPlayers(players) {
      ...
      if (!players || players.length === 0) {
         tableBody.innerHTML = '<tr><td colspan="6" style="text-align:center;">Nema podataka za prikaz</td></tr>';
         return;
      }

      players.forEach((player) => {
         const row = document.createElement('tr');
         const playerLink = player.playerUrl || '#';
         row.innerHTML = `
        <td>${player.redni_broj || 'N/A'}</td>
        <td>
          <img src="${player.playerImage || 'https://via.placeholder.com/50'}" alt="${player.ime || ''}" class="player-img" />
          <a href="${playerLink}" target="_blank" style="color: white;">${player.ime || 'N/A'} ${player.prezime || ''}</a>
        </td>
        <td><img src="${player.teamLogo || 'https://via.placeholder.com/30'}" alt="${player.tim || ''}" class="team-logo">${player.tim || 'N/A'}</td>
        <td>${player.pozicija || 'N/A'}</td>
        <td>${player.visina || 'N/A'}</td>
        <td>${player.tezina || 'N/A'}</td>
      `;
   ...
   searchInput.addEventListener('input', () => {
      ...
      const filtered = allPlayers.filter(player =>
         `${player.ime} ${player.prezime}`.toLowerCase().includes(query)
      );
...
// Handle NBA Knowledge link click
document.addEventListener('DOMContentLoaded', () => {
   const nbaKnowledgeLink = document.querySelector('.nba_knowledge_link');

   if (nbaKnowledgeLink) {
      nbaKnowledgeLink.addEventListener('click', (event) => {
         event.preventDefault(); // Prevent the default link behavior
         if (localStorage.getItem('isLoggedIn') === 'true') {
            window.location.href = 'profile.html'; // Redirect to profile if logged in
         } else {
            window.location.href = 'sign_in.html'; // Redirect to sign-in if not logged in
         }
      });
   }
});
