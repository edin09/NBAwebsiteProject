...
toggleButtons.forEach(button => {
   button.addEventListener('click', () => {
      ...
      // Onemogući sve dugmiće iz iste grupe
      const allButtons = button.parentElement.querySelectorAll('.toggle-button');
      allButtons.forEach(btn => btn.classList.remove('active'));
      button.classList.add('active');

      // Prikazuj samo ciljani sadržaj
      const wrapper = button.closest('.leaders-wrapper');
      const cards = wrapper.querySelectorAll('.leaders-card');
      cards.forEach(card => card.classList.add('hidden'));

      const targetCard = wrapper.querySelector(`#${target}`);
      if (targetCard) {
