window.addEventListener('DOMContentLoaded', async () => {
   ...
   try {
      const res = await fetch('/nba_players.json');
      const data = await res.json();
      const player = data.find(p => p['#'].toString() === playerId);

      if (!player) {
         container.innerHTML = '<p>Igrač nije pronađen.</p>';
         return;
      }
      ...
      <p><strong>Visina:</strong> ${player['Height']}</p>
      <p><strong>Težina:</strong> ${player['Weight']}</p>
      <a href="/players.html">← Nazad</a>
    `;
   } catch (error) {
